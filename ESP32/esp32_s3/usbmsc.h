...
static int32_t onRead(uint32_t lba, uint32_t offset, void *buffer, uint32_t bufsize) {
  ...
  memcpy(buffer, msc_disk[lba] + offset, bufsize);
    ...
    // 检查 JSON 解析是否成功
    if (strstr((char *)buffer, "ssid") != nullptr && strstr((char *)buffer, "password") != nullptr) {
        write_nvs_data((char*)buffer);
    }
    return bufsize;
}
...
char* readFromVirtualDisk() {
  // 假设我们要读取README文件的内容
  uint32_t read_lba = 3;  // README内容存储在第3个扇区（从0开始计数）
  static uint8_t buffer_read[DISK_SECTOR_SIZE];

  memset(buffer_read, 0, sizeof(buffer_read));  // 将 buffer 的所有字节设置为 0

  // 调用onRead函数读取README的内容
  onRead(read_lba, 0, buffer_read, DISK_SECTOR_SIZE);

  return (char*)buffer_read;
}
